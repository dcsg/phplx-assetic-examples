    ...
    (function (window, undefined) {
            ...
            for (; i < len; i++) {
                if ((matcher = Expr.relative[tokens[i].type])) {
                    ...
                    if (matcher[expando]) {
                        ...
                        return setMatcher(
                            i > 1 && elementMatcher(matchers), i > 1 && tokens.slice(0, i - 1).join("").replace(rtrim, "$1"), matcher, i < j && matcherFromTokens(tokens.slice(i, j)), j < len && matcherFromTokens((tokens = tokens.slice(j))), j < len && tokens.join(""));
                    }
        ...
        function select(selector, context, results, seed, xml) {
                ...
                if (match.length === 1) {
                    ...
                    for (i = matchExpr["POS"].test(selector) ? -1 : tokens.length - 1; i >= 0; i--) {
                        ...
                        if ((find = Expr.find[type])) {
                            ...
                            if ((seed = find(
                                token.matches[0].replace(rbackslash, ""), rsibling.test(tokens[0].type) && context.parentNode || context, xml))) {
            ...
            compile(selector, match)(
                seed, context, xml, results, rsibling.test(selector));
            return results;
        }
